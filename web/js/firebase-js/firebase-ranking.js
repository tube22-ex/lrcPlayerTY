...
function show_ranking(id){
    ...
    async function DBdataSelector(dbdata){
        ...
        for (const Array of rank_Array) {
              ...
              const liHTML = `<li id="${Array["NAME"]}"><span class="ranking_score">${toFixedScore}</span><img src="${data["iconURL"]}" class="ranking_icon"><span class="ranking_name">${data["dispName"]}<span></li>`;
              
              document.getElementById('ranking_ol').insertAdjacentHTML("beforeend", liHTML);
        }
        /*
        rank_Array.forEach((Array)=>{
            getProfile(Array["NAME"]).then(data =>{
                console.log(data)
                tofinxedScore = (Math.round((Array["Score"] / 2000)*100)/100).toFixed(2); 
                document.getElementById('ranking_ol').insertAdjacentHTML("beforeend",`<li id="${Array["NAME"]}">${tofinxedScore} <img src="${data["iconURL"]}">${data["dispName"]}</li>`);
            });
        })
        */
